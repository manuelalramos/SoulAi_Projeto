...
// Props usadas para controlar a abertura do menu em telas menores.
type NavMenuProps = {
  menuAberto: boolean;
  fecharMenu: () => void;
};
...
export function NavMenu({ menuAberto, fecharMenu }: NavMenuProps) {
  ...
  return (
    <ul className={menuClass}>
      {/* Gera os links do menu e aplica destaque na rota ativa. */}
      {navigationItems.map((item) => (
        <li key={item.path}>
          <NavLink
            ...
            onClick={fecharMenu}
            className={({ isActive }) => {
              // Classe base compartilhada entre links ativos e inativos.
              const baseClass =
